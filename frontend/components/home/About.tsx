...
export default function About() {
  ...
  const scrollPrev = () => {
    ...
    if (container.scrollLeft <= 10) {
      // Прокручиваем так, чтобы правый край был видим
    ...
    <section id="about" className="relative z-0 w-full overflow-hidden bg-[#F5F2EB]">
      ...
      <div className="absolute inset-0 -z-10 w-full h-full pointer-events-none" aria-hidden="true">
        <Image
          ...
          alt={tAbout("parchmentAlt") ?? "Parchment background"}
          fill
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at center, transparent 60%, rgba(43,30,23,0.015) 100%)",
          }}
          aria-hidden="true"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 w-full border-b border-brand-dark/5">
        ...
        <div className="flex flex-col justify-center px-6 py-12 sm:px-12 md:px-20 lg:px-28 text-brand-dark max-w-2xl justify-self-end w-full">
          <div className="flex items-center gap-3 mb-4 md:mb-6">
            <span className="font-sans text-xs tracking-[0.2em] uppercase text-brand-gold font-bold">
              {tMenu("subtitle")}
            </span>
          </div>
          <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl uppercase tracking-wide mb-4 md:mb-6">
            {tMenu("title")}
          </h2>
          <p className="font-sans text-sm md:text-base leading-relaxed text-brand-dark/80 mb-6 md:mb-8 font-light">
            {tMenu("description")}
          </p>

          <Link
            href="/menu"
            className="group inline-flex items-center gap-2 font-sans text-xs tracking-[0.15em] uppercase font-semibold text-brand-gold hover:text-brand-red transition-colors w-fit leading-none font-bold"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 w-full border-b border-brand-dark/5">
        ...
        <div className="relative h-[300px] sm:h-[400px] md:h-auto min-h-[350px] md:min-h-[450px] w-full overflow-hidden">
          <Image
            ...
            alt={tAbout("interiorAlt") ?? "Interior"}
            fill
            className="object-cover transition-transform duration-700 hover:scale-105"
          />
        </div>
      ...
      <div className="max-w-[1440px] mx-auto px-6 py-16 sm:px-12 md:px-20 lg:px-28 xl:px-36">
        <div className="max-w-3xl text-brand-dark mb-10">
          <span className="font-sans text-xs tracking-[0.2em] uppercase text-brand-gold font-bold mb-3 block">
            {tInterior("subtitle")}
          </span>
        ...
        >
          {interiorImages.map((src, index) => (
            ...
            >
              <Image
                src={src}
                alt={tInterior("imageAlt", { number: index + 1 }) ?? `Interior photo ${index + 1}`}
                fill
        ...
        <div className="hidden sm:flex justify-between items-center mt-8">
          ...
          <button
            ...
            aria-label={tInterior("prevAria") ?? "Previous items"}
          >
            <svg
          ...
          <button
            onClick={scrollNext}
            className="group flex items-center justify-center w-12 h-12 rounded-full border border-brand-dark/10 hover:border-brand-gold text-brand-dark hover:text-brand-gold transition-all duration-300 cursor-pointer select-none"
            aria-label={tInterior("nextAria") ?? "Next items"}
          >
            <svg
              className="w-5 h-5 transform translate-x-0 group-hover:translate-x-0.5 transition-transform duration-300"
              viewBox="0 0 24 24"
      ...
      {/* Локальные стили для скрытия скроллбара в WebKit */}
      <style jsx>{`
        .hide-scrollbar {
